...
const PriceRangeCard = ({ title, data, criterion, onDataChange, onAdd, onRemove }: PriceRangeCardProps) => (
    <div className="bg-white rounded-2xl border border-gray-200 p-6">
            ...
            {data.map((item, index) => (
                <div key={item.id || `new-${index}`} className="grid grid-cols-10 gap-4 items-center text-sm">
                    <input 
                        ...
                        className="col-span-3 text-gray-800 text-center bg-white rounded-md p-2 border border-gray-300 focus:outline-none focus:ring-1 focus:ring-brand-red"
                    />
                    <input 
                        type="number" 
                        placeholder={criterion === 'pages' ? 'Págs' : '%'}
                    ...
                    <input 
                        ...
                        className="col-span-3 font-medium text-gray-900 text-center bg-white rounded-md p-2 border border-gray-300 focus:outline-none focus:ring-1 focus:ring-brand-red"
                    />
        ...
        <button onClick={() => onAdd(criterion)} className="flex items-center gap-2 text-[#FF5A5A] mt-6 font-medium text-sm">
            <PlusCircle size={20} />
    ...
    <div className="bg-white min-h-screen font-inter text-black p-6 md:p-10">
        <div className="max-w-3xl">
            ...
            <footer className="mt-10 flex justify-end gap-4">
                ...
                <button onClick={handleSaveChanges} className="bg-[#FF5A5A] text-white py-3 px-8 rounded-full font-semibold" disabled={saving}>
