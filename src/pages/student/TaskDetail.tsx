            ...
            {timeline.map((step, index) => (
              <div key={index} className="flex items-start relative pb-6 last:pb-0">
                <div className="flex-shrink-0 mr-4">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center ${step.is_current ? 'bg-primary' : 'border-2 border-gray-300 bg-white'}`}>
                ...
                <div>
                  <p className={`font-medium text-sm ${step.is_current ? 'text-primary' : 'text-gray-700'}`}>{step.title}</p>
